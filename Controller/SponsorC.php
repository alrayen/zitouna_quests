...
class SponsorC {
    ...
    public function supprimerSponsor($id) {
        ...
        try {
            ...
            $sqlDons = "DELETE FROM donation WHERE sponsor_id = :id";
            $queryDons = $db->prepare($sqlDons);
            $queryDons->bindValue(':id', $id);
            $queryDons->execute();

            // Then delete the sponsor
            $sqlSponsor = "DELETE FROM sponsor WHERE id = :id";
            $querySponsor = $db->prepare($sqlSponsor);
            $querySponsor->bindValue(':id', $id);
