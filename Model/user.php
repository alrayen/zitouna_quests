...
class User
{
    ...
    public string $rank;
    public function __construct(string $nom, string $prenom, string $datenaissance, string $email, string $password,string $role, int $niveau, int $pointtotal, string $photo, string $badge, string $rank,string $bio,int $etat=0)
    {
        $this->nom = $nom;
        $this->prenom = $prenom;
    ...
    try {
        ...
        if ($user && password_verify($password, $user['password'])) {
            if (isset($user['etat']) && $user['etat'] == 1) {
                // User is banned, return a specific string to indicate this
                return 'banned';
            }
            return $user; 
        }
        ...
        // It's better to log the error and not expose details to the user.
...
public static function banUser($id) {
    try {
        ...
        // Idéalement, enregistrez l'erreur dans un fichier journal
        // echo "Erreur lors du bannissement: " . $e->getMessage();
        return false;
    }
}
...
public static function unbanUser($id) {
    try {
        ...
        // Idéalement, enregistrez l'erreur dans un fichier journal
        // echo "Erreur lors du débannissement: " . $e->getMessage();
        return false;
    }
}
