...
if (isset($_POST['nom'])) {
    ...
    header('Location: listSponsor.php?status=updated');
    exit();
}
?>
      ...
      <nav class="relative flex flex-wrap items-center justify-between px-0 py-2 mx-6 transition-all ease-in shadow-none duration-250 rounded-2xl lg:flex-nowrap lg:justify-start">
        <div class="flex items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit">
          <nav>
            ...
            <h6 class="mb-0 font-bold text-white capitalize">Edit Sponsor</h6>
          </nav>
        </div>
      </nav>
        ...
        <div class="flex flex-wrap -mx-3">
          <div class="w-full max-w-full px-3 mx-auto mt-0 md:flex-0 shrink-0 md:w-7/12">
            <div class="relative flex flex-col min-w-0 break-words bg-white border-0 shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
              <div class="p-6 pb-0 mb-0 border-b-0 border-b-solid rounded-t-2xl border-b-transparent bg-gradient-to-r from-orange-50 to-white">
                <h6 class="dark:text-white font-bold text-lg text-slate-700">Modify Sponsor #<?php echo $old['id']; ?></h6>
              </div>
              <div class="flex-auto p-6">
                <form role="form" action="" method="POST" id="sponsorForm" novalidate>
                  <div class="mb-4">
                    ...
                    <input type="text" name="nom" id="nom" value="<?php echo htmlspecialchars($old['nom']); ?>" class="focus:shadow-primary-outline dark:bg-slate-850 dark:text-white text-sm leading-5.6 ease block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-orange-500 focus:outline-none" />
                  ...
                  <div class="mb-4">
                    ...
                    <select name="secteur" id="secteur" class="focus:shadow-primary-outline dark:bg-slate-850 dark:text-white text-sm leading-5.6 ease block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 outline-none transition-all focus:border-orange-500 focus:outline-none">
                        ...
                        <option value="Other" <?php if($old['secteur']=='Other') echo 'selected'; ?>>🔹 Other</option>
                    </select>
                  ...
                  <div class="mb-4">
                    ...
                    <input type="email" name="contact" id="contact" value="<?php echo htmlspecialchars($old['contact']); ?>" class="focus:shadow-primary-outline dark:bg-slate-850 dark:text-white text-sm leading-5.6 ease block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-orange-500 focus:outline-none" />
                    <span id="errorContact" class="text-red-500 text-xs font-bold mt-1 block"></span>
                  </div>
                  <div class="mb-4">
                    <label class="block mb-2 ml-1 text-xs font-bold text-slate-700 uppercase">Contribution (DT)</label>
                    <input type="number" name="contribution" id="contribution" value="<?php echo htmlspecialchars($old['contribution']); ?>" class="focus:shadow-primary-outline dark:bg-slate-850 dark:text-white text-sm leading-5.6 ease block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-orange-500 focus:outline-none" />
                    <span id="errorContribution" class="text-red-500 text-xs font-bold mt-1 block"></span>
                  </div>
                  <div class="text-center">
                    <button type="submit" id="submitBtn" class="inline-block w-full px-16 py-3.5 mt-6 mb-0 font-bold text-center text-white uppercase align-middle transition-all border-0 rounded-lg cursor-pointer hover:scale-102 active:opacity-85 hover:shadow-lg leading-pro text-xs ease-in tracking-tight-rem shadow-md bg-gradient-to-tl from-orange-500 to-yellow-500">Update Sponsor</button>
                    <a href="listSponsor.php" class="inline-block w-full px-16 py-3.5 mt-3 mb-0 font-bold text-center text-slate-700 uppercase align-middle transition-all border-0 rounded-lg cursor-pointer hover:scale-102 active:opacity-85 leading-pro text-xs ease-in tracking-tight-rem bg-gray-100">Cancel</a>
                  </div>
                </form>
              </div>
            </div>
    ...
    <script>
        document.getElementById('sponsorForm').addEventListener('submit', function(e) {
            ...
            if (contribution.value.trim() === "") {
                errorContribution.innerText = "Contribution is required.";
                isValid = false;
