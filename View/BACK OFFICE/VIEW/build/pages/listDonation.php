      ...
      <nav class="relative flex flex-wrap items-center justify-between px-0 py-2 mx-6 transition-all ease-in shadow-none duration-250 rounded-2xl lg:flex-nowrap lg:justify-start">
        <div class="flex items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit">
          <nav>
            ...
            <h6 class="mb-0 font-bold text-white capitalize">Donations History</h6>
          </nav>
        </div>
      </nav>
                ...
                <div class="flex-auto p-4">
                  <div class="flex flex-row -mx-3">
                    <div class="flex-none w-2/3 max-w-full px-3">
                      <div>
                        <p class="mb-0 font-sans text-sm font-semibold leading-normal uppercase dark:text-white dark:opacity-60">Total Donations</p>
                        <h5 class="mb-2 font-bold dark:text-white"><?php echo $totalDonations; ?></h5>
                      </div>
                    </div>
          ...
          <div class="flex-none w-full max-w-full px-3">
            <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border overflow-hidden">
              <div class="p-6 pb-0 mb-0 border-b-0 border-b-solid rounded-t-2xl border-b-transparent bg-gradient-to-r from-gray-50 to-white">
                <h6 class="dark:text-white font-bold text-lg text-slate-700"><i class="fas fa-history mr-2 text-blue-500"></i> Donation Records</h6>
              </div>
                  ...
                  <table class="items-center w-full mb-0 align-top border-collapse dark:border-white/40 text-slate-500">
                    <thead class="align-bottom">
                      <tr class="bg-gray-50 text-left">
                        <th class="px-6 py-3 font-bold text-left uppercase align-middle bg-transparent border-b border-collapse shadow-none text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400 opacity-70">Sponsor / Donator</th>
                        <th class="px-6 py-3 pl-2 font-bold text-left uppercase align-middle bg-transparent border-b border-collapse shadow-none text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400 opacity-70">Type & Amount</th>
                        <th class="px-6 py-3 font-bold text-center uppercase align-middle bg-transparent border-b border-collapse shadow-none text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400 opacity-70">Date</th>
                    ...
                    <tbody>
                      <?php if (!empty($liste)): ?>
                          ...
                          <tr class="hover:bg-gray-50 transition-all duration-200 border-b border-gray-100">
                            <td class="p-4 align-middle bg-transparent whitespace-nowrap shadow-transparent">
                              <div class="flex px-2 py-1">
                                <div>
                                  <div class="inline-flex items-center justify-center mr-4 text-sm text-white h-10 w-10 rounded-full shadow-md bg-gradient-to-tl from-emerald-500 to-teal-400 ring-2 ring-white">
                                    <i class="fas fa-hand-holding-heart text-xs"></i>
                                  </div>
                                </div>
                                <div class="flex flex-col justify-center">
                                  <h6 class="mb-0 text-sm font-bold leading-normal dark:text-white text-slate-700"><?php echo htmlspecialchars($row['nom_donateur']); ?></h6>
                                </div>
                              </div>
                            </td>
                            
                            <td class="p-2 align-middle bg-transparent whitespace-nowrap shadow-transparent">
                                <p class="mb-0 text-xs font-semibold leading-tight"><?php echo htmlspecialchars($row['type_don']); ?></p>
                                <p class="text-xs text-emerald-500 font-bold mb-0"><?php echo htmlspecialchars($row['montant']); ?> DT</p>
                            </td>
                            
                            <td class="p-2 text-center align-middle bg-transparent whitespace-nowrap shadow-transparent">
                                <span class="text-xs font-semibold text-slate-400"><?php echo $row['date_don']; ?></span>
                            </td>
                            
                            <td class="p-2 text-center align-middle bg-transparent whitespace-nowrap shadow-transparent">
                                <?php if($row['etat'] == 'En attente'): ?>
                                    <span class="bg-orange-100 text-orange-600 px-2.5 py-0.5 rounded-full text-xs font-bold"><i class="fas fa-clock mr-1"></i> Pending</span>
                                <?php else: ?>
                                    <span class="bg-emerald-100 text-emerald-600 px-2.5 py-0.5 rounded-full text-xs font-bold"><i class="fas fa-check-circle mr-1"></i> Validated</span>
                                <?php endif; ?>
                            </td>

                            <td class="p-2 text-center align-middle bg-transparent whitespace-nowrap shadow-transparent">
                                <?php if($row['etat'] == 'Validé'): ?>
                                    <span class="badge bg-blue-100 text-blue-600 rounded-full px-2 py-1 text-xs font-bold">+<?php echo $row['points_gagnes']; ?> pts</span>
                                <?php else: ?>
                            ...
                            <td class="p-2 text-center align-middle bg-transparent whitespace-nowrap shadow-transparent">
                                <div class="flex justify-center items-center gap-2">
                                    <?php if($row['etat'] == 'En attente'): ?>
                                        <a href="validateDonation.php?id=<?php echo $row['id']; ?>&montant=<?php echo $row['montant']; ?>" class="w-8 h-8 rounded-full bg-blue-100 text-blue-600 hover:bg-blue-500 hover:text-white flex items-center justify-center transition-all shadow-sm" title="Validate Donation">
                                            <i class="fas fa-check text-xs"></i>
                                        </a>
                                    <?php endif; ?>
                                    <button type="button" onclick="confirmDelete(<?php echo $row['id']; ?>)" class="w-8 h-8 rounded-full bg-gray-100 hover:bg-red-500 hover:text-white flex items-center justify-center transition-all shadow-sm cursor-pointer border-none" title="Delete Donation">
                          ...
                          <?php endforeach; ?>
                      <?php else: ?>
                          <tr><td colspan="6" class="text-center p-8 text-gray-400">No donations found.</td></tr>
                      <?php endif; ?>
                    </tbody>
                  </table>
                </div>
        ...
        function confirmDelete(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = 'deleteDonation.php?id=' + id;
                }
            })
        }
    </script>
