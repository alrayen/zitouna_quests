        ...
        <div class="container">
            <div class="row">
                <div class="col-12 d-flex justify-content-center">
                    <form class="login-wrapper registration"  Method="POST" action="../../../../Controller/registrer.php">
                        <!-- <div class="mb-5">
                        ...
                        <div class ="mb-5">
                            <label for="exampleInputEmail3" class="form-label">birthdate</label>
                            <input type="date" id="dateNaissance">
                            <p id="erreurdateNaissance" style="color:red;"></p>
                        </div>  
                        <div class="mb-5">
                        ...
                        <div class="mb-5">
                            ...
                            <p id="erreurpassword2" style="color:red;"></p>
                        </div>
                        <div class="mb--10">
                            <input type="checkbox" id="remember">
                            <label style="text-transform: bold" for="exampleCheck1">Accepter nos temes et condition d utilisation</label>
                            
                        </div>
                        <button type="submit" class="rts-btn btn-primary radious-5 mr--15">Sign Up</button>
...
=    <div class="mb-5">
        <label class="form-label">Reconnaissance Faciale (Optionnel)</label>
        <p style="color: rgba(255, 255, 255, 0.6); font-size: 13px; margin-bottom: 15px;">
            Enregistrez votre visage pour une connexion plus rapide et sécurisée.
        </p>
        <div id="face-capture-container" class="face-capture-wrapper" style="display: flex; align-items: center; justify-content: center;">
            <video id="video" width="320" height="240" autoplay muted style="display: none; object-fit: cover;"></video>
            <div class="face-overlay"></div>
            <button type="button" id="start-face-capture" class="rts-btn btn-secondary radious-5">Enregistrer mon visage</button>
            <p id="face-status" style="color: #fff; font-weight: bold; display: none;"></p>
        </div>
    ...
    <div class="mb--10">
        <input type="checkbox" id="remember" >
        <label for="remember">Accepter nos termes et conditions d'utilisation</label>
    </div>
    <div class="mb-4">
    ...
    <?php if (isset($_SESSION['error_message'])): ?>
        <p style="color: #ff6b6b; text-align: center; margin-bottom: 15px; background: rgba(255, 107, 107, 0.2); padding: 10px; border-radius: 5px;"><?php echo $_SESSION['error_message']; unset($_SESSION['error_message']); ?></p>
    <?php endif; ?>

    <button type="submit" class="rts-btn btn-primary radious-5 mr--15" style="color: #fff;">Sign Up</button>
    ...
    <div class="g_id_signin" data-type="standard" data-size="large" data-theme="outline" data-text="continue_with" data-shape="rectangular" data-logo_alignment="left"></div>
        ...
        <i class="fab fa-github" style="margin-right: 8px;"></i> Continuer avec GitHub
    </a>
                    </form>
                </div>
            </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const startButton = document.getElementById('start-face-capture');
            ...
            async function loadModels() {
                try {
                    ...
                    console.error('Erreur lors du chargement des modèles de face-api:', error);
                    setStatus("Impossible de charger les modèles de reconnaissance faciale. Veuillez vérifier votre connexion internet et rafraîchir la page.", '#FF6B6B');
            ...
            startButton.addEventListener('click', async () => {
                if (!modelsLoaded) {
                    setStatus('Les modèles sont encore en cours de chargement, veuillez patienter...');
                    return;
                }
                ...
                document.querySelector('#face-capture-container .face-overlay').style.display = 'block';
                ...
                try {
                    ...
                    const detectionInterval = setInterval(async () => {
                        const detections = await faceapi.detectSingleFace(video, new faceapi.SsdMobilenetv1Options({ minConfidence: 0.4 })).withFaceLandmarks().withFaceDescriptor();
                        ...
                        if (detections) {
                            ...
                            document.querySelector('#face-capture-container .face-overlay').style.display = 'none';

                            const descriptorArray = Array.from(detections.descriptor);
                            const truncatedDescriptor = descriptorArray.map(num => parseFloat(num.toFixed(10)));
                            const descriptorString = JSON.stringify(truncatedDescriptor);
                            faceDescriptorInput.value = descriptorString;

                            console.log("--- SIGN UP DESCRIPTOR ---", descriptorString);
                            setStatus('Visage enregistré avec succès !', '#00E6A7');
                            console.log('Descripteur facial enregistré.');
                        } else {
                            setStatus('Aucun visage détecté. Essayez de vous rapprocher...');
                        }
                    }, 500);
