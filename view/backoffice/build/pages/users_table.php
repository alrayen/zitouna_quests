...
// Handle EDIT (Update) from the modal form
if (isset($_POST['update_user'])) {
    ...
    $role = $_POST['edit_role']; // Assuming you might want to edit the role too

    // Note: The User class's updateProfile method seems built for front-end profile updates
    // (with password/bio/birthdate), but for an admin table, we'll call a simplified
    // update method on the controller if available, or try to map to what is available.
    // Assuming you have an admin-specific update method in your controller:
    $result = $userController->adminUpdateUser($userId, $nom, $prenom, $email, $niveau, $points, $role);
    
    // Redirect after update
    if ($result['success'] ?? true) { // Adjust logic based on your controller's adminUpdateUser response
        header("Location: users_table.php?status=success");
        exit();
    } else {
        // Handle error, e.g., show a message on the page
        // For simplicity, we'll just redirect without a message
...
<head>
  ...
  <ul class="flex flex-col pl-0 mb-0">
    ...
    data-user='<?php echo htmlspecialchars(json_encode([
        ...
        'nom' => $user['nom'],
        'prenom' => $user['Prenom'], // La clé est déjà 'prenom', c'est parfait.
        'email' => $user['email'],
        'niveau' => $user['niveau'],
        'points' => $user['points'],
        'role' => $user['role']
